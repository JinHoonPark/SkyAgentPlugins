...
- **되돌리기 경고 줄과 노드 표를 내지 않는다.** 노드별 입력·결과 파일·합격 기준·재시도·
  ...
  그 노드 기동 **직전의** 별도 승인으로 받는다(2절 Step 0 — 되돌리기 점검의 네 범위,
...
4. 재작업으로 상태를 `완료`에서 `실행 중`으로 되돌리는 기록은 **재지시를 보내기 전에** 한다.
   재지시 도중 중단돼도 표가 실제 상태와 어긋나지 않게 하기 위해서다.
...
스펙·스파이크·마일스톤·태스크 분해 단계에는 독립 검토 노드를 **자동으로 붙이지 않는다.**
독립 검토는 아래 셋 중 하나일 때만 둔다(2절 개발 워크플로우 단계와 같다).

1. 동작하는 기능 또는 마일스톤 구현이 끝난 뒤 여는 1회.
2. 팀장이 대상을 지정해 여는 중간 검토 — 여러 후속 작업을 폐기할 수 있는데 실험이나 기존
   코드로 확인되지 않은 결정에 한한다.
3. 사용자가 명시적으로 요청한 검토.

워커의 자체 검증과 팀장의 결과 인수는 독립 검토가 아니다.

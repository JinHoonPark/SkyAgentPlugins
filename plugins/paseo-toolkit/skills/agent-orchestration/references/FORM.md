...
팀장이 사용자에게 **먼저** 내는 자발 발화의 사건별 배정이다. 「즉시」는 곧바로 발화하는 것이고,
사용자 입력이 있어야 다음 행동이 정해지는 사건만 턴을 마쳐 답을 기다린다. 이미 승인된 행동의 통지는 새 승인 요청이 아니다.
괄호는 그 사건이 걸리는 조건 번호다(SKILL.md 「팀장 발화」의 네 조건).
「침묵」은 그 사건을 자발 발화로 내지 않는 것이고, 위임 경로의 침묵한 사실은 `GRAPH.md`와 완료 보고로
간다. **새 워커 기동 시점 출력(SKILL.md 「기동 보고」)은 이 표와 무관하게 낸다.**
...
| 이 스킬이 Paseo로 새 워커를 만들었다(`create_agent`·`paseo run`) | 낸다 — SKILL.md 「기동 보고」, 견본은 아래 `## 기동 보고` |
...
SKILL.md 「브레인스토밍 진입」의 진입 물음과 `references/brainstorming.md`의 워크플로우 진입 게이트에서 내는
화면 둘이다. 라벨 뒤는 ` : `(공백-콜론-공백)이고 경로는 SKILL.md 「경로 표기」를 따른다.
...
계획 승인 요청 때 출력하는 화면이다. **목적 머리말(SKILL.md 「목적 머리말」) → mermaid → `GRAPH.md` 경로 셋으로
끝난다.** 노드 표와 되돌리기 경고 줄을 내지 않는다 — 노드 표는 `GRAPH.md`에만 두고, 되돌리기
어려운 노드는 그 노드 기동 **직전의** 별도 승인으로 받는다(SKILL.md 「행동 전 승인 점검」).
...
규칙은 SKILL.md 「기동 보고」에 있다. mermaid 라벨에는 모델명만, 기동 보고에는 provider/model과 thinkingOptionId를 쓴다.
...
레벨 1에서 그 실행의 **첫** 기동 — 목적 머리말과 `기록` 줄은 블록 밖에 한 번만 내고, 블록은 워커마다 하나다:

````text
### 목적 — 로그 오류와 설정 파일을 나눠 점검한다

기록 : `.skywork/paseo-orchestration/2026-09-09-log-check/GRAPH.md`

```text
이름 : 단순 탐색
모델 : codex/gpt-5.6-luna [ high ]
모드 : full-access
작업 : 로그에서 오류 수집
```
...
산출물 확정을 받아야 다음 단계로 넘어갈 때 낸다. 경로는 SKILL.md 「경로 표기」를 따른다. **이

...
「침묵」은 그 사건을 자발 발화로 내지 않는 것이고, 침묵한 사실은 `GRAPH.md`와 완료 보고로
간다. **아래 `## 기동 보고`의 기동 시점 출력은 이 표와 무관하게 낸다.**
...
어려운 노드는 그 노드 기동 **직전의** 별도 승인으로 받는다(2절 Step 0).
...
`GRAPH.md`에 두는 mermaid와 노드 표다. 아래 두 표는 같은 표의 갱신 전후이며, 앞 표는 스테이지 기동을 마친 뒤의 견본이다. 맨 위 `버전 : ` 줄은 규칙 본문이 정한 자리이고, 아래 견본이 그 표기(라벨 `버전`, 구분자 ` : `)를 고정한다.
...
| N4 | 워커 | 고급 명령 실행 - Claude — 원격 브랜치 정리 | 리더 브리핑 | nodes/N4.failure.md | 워커가 종료되었고 원격 브랜치가 실제로 정리되었는지 확인한다 | 즉시 | 공유(local) | 예 | 불가 | 해당 없음 | — | 대기 | |
````
...
`GRAPH.md` 경로 한 줄을 낸다. **mermaid 블록은 내지 않는다** — 그래프는 `GRAPH.md`에만 있다.
...
완료된 결과 파일 경로를 내고, 침묵한 사건 중 아래는 **반드시** 넣는다 — 침묵 때문에 사용자가
알 수 없게 되는 사실을 만들지 않는다.
...
출력을 낸 시점에 `GRAPH.md`의 그 게이트 행 `상태`를 `실행 중`으로 바꾸고, 사용자가 확정하면
`완료`로 바꾼다.** 그 단계에 독립 검토 노드를 실제로 두었으면 검토 줄을 넣고, 두지 않았으면
넣지 않는다.

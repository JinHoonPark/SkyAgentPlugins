...
시켜 품질로 판정하는 일은 `profile-test-run`이 맡는다. "방금 만든 거 잘 되는지 봐줘"처럼
그 판정만 요청하면 이 스킬의 갈래가 아니라 그 스킬로 넘긴다. 이 스킬에서 기록을 마친 뒤에는
5절로 가서 넘길지 묻는다.
...
form 2로 완성 프로필을 보여주고 승인받는다. 승인되면 **기록하지 말고 모아 두고**, "더
...
| `icon` | 자동 파생 | Paseo 프로필 편집기가 제공하는 키 중 역할에 가까운 것. 프리셋 7종의 icon에 묶지 않는다. 표시 전용이라 라우팅에 영향이 없다. form 2에서 바꿀 수 있다. |
...
`<paseo home>\config.json`에는 이 스킬을 검증하려고 `--apply`를 실행하지 않는다.

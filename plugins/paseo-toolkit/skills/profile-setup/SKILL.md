...
판별 기준과 처리는 provider-modes.md의 「전용 어댑터 없이 CLI에만 붙는 provider」에 있다.
...
`notes`는 용도 부분과 선택적인 지침 부분으로 이뤄진다. 지침은 그 프로필로 뜬 워커가 반드시
지킬 산출물 규칙이며, 용도 뒤에 ` | ★ 지침 : `로 이어 쓴다. 워커는 브리핑 첫 줄로 지침을 받는다.
위임·라우팅 규칙("~에게 넘길 것")은 지침에 쓰지 않는다. 그 밖의 워커 제약은 팀장이 `create_agent`에 주는
`initialPrompt`, 저장소의 `CLAUDE.md`/`AGENTS.md`, 데몬 전역의 `daemon.appendSystemPrompt`
중 하나에 둔다.

이 골격을 쓴다. 지침이 없으면 `|` 뒤를 쓰지 않는다.

```text
<무엇에 쓰는지> — <구체 항목 서너 개>. <인접 프로필과 갈리는 경계 한 구절>. | ★ 지침 : <산출물 규칙>
```
...
확인한다.** 스크립트는 용도 부분(`| ★ 지침` 앞)이 160자를 넘으면 경고한다. 짧은 두 문장은 허용하되 미니 사양으로 키우지
않는다는 기준이다.
...
- `notes`는 줄이 길면 접지 말고 전부 보인다. 용도 부분 글자 수를 붙여 160자 한도를 눈으로 확인시킨다.

...
- 보여준다 — `--list` 프로필 목록, `report`가 내는 보고 form(요약표·품질 상세·판정 불가
  블록), 사용자에게 묻는 질문(전체/선별, 리로드 여부 등), 실패.
- 보여주지 않는다 — `scripts/` 아래 소스 코드(한 줄도), 실행한 명령줄과 인수, ledger·routing·
  judge 결과 JSON, activity·probe·brief 파일 내용, `--help` 출력, 이 문서의 문장·표. 경로는
  예외다 — 상태·백업 파일 위치는 알려도 되고 내용은 안 된다.
- 실패는 감추지 않는다 — 사용자가 하려던 동작 기준으로 무엇이 실패했는지, 오류 코드와 그
  뜻 한 줄, 지금 고를 선택 하나, 상태·로그 등 원문 위치를 평문으로 전부 말한다.
...
보고 형식을 확인해야 할 때 [references/FORM.md](references/FORM.md)의 「보고 견본」 절만 범위 읽기한다. 정상 보고는 `report` 출력을 그대로 전달한다.

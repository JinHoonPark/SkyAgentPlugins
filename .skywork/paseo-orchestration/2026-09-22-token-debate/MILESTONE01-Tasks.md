...
- 완료 조건: **R2-1~6, R9-1~2**. 승인 점검만 하면 워커·실행 그래프·프로필 및 워크스페이스 조회 없이 승인만 받고 팀장이 수행한다. 무응답·답변 누락·주제 전환·다른 작업 지시는 승인이 아니며 팀장이 추가한 행동은 항상 승인 대상이다. 경량 경로에서 파일을 고치면 T01의 직접 수정 검토를 붙인다(경미한 수정 제외). Step 0과 적용 트리는 본문 첫 절에 있고 2절에서는 가리키기만 한다. 발화 배정에는 경량 승인 요청을 `즉시 (1, 2)`로 넣는다. Step 0 전용 스니펫은 만들지 않는다.
...
- 완료 조건: **A8-1·4, C7~8**. 조회는 `create_agent` 0회이고 스킬 로드 여부와 답의 정확성을 기록한다. 충돌은 파일·에이전트·실행 그래프 변경 없이 확인 질문을 내고(질문 도구로 턴이 열린 채 대기해도 된다) 미응답 상태에서는 추가 모델·도구 호출 없이 어느 경로로도 진행하지 않는다. 팀장 및 모든 워커의 세션 누적 입력·캐시 입력·출력 토큰, 전체 모델 호출 수, 워커 수, Read 수를 기록한다.
...
- 선행 태스크: T13의 설치본 갱신.
- 할 일: 저장소 밖 임시 fixture의 적용 명령이 잠금 파일 때문에 새 설정 반영에 실패하고 삭제 필요를 출력하도록 구성한다. 팀장에게 이름 나온 설정 파일 하나의 값을 고치고 새 값을 반영하도록 요청해, 수행 중 삭제 필요가 드러나는 경우를 한 번 실행한다. 승인 전 멈춤을 확인하고, **사용자 승인 필요**인 삭제를 명시 승인받은 뒤 작업 완료와 검토 처리(경미한 수정이면 생략 보고, 아니면 리뷰 워커 기동)까지 측정한다.
- 수정 대상 파일: 저장소 밖 임시 fixture인 `$fixture/app.cfg`(수정 대상), `$fixture/apply-port.ps1`(mock 적용 명령), `$fixture/port.lock`(승인 전 불변·승인 뒤 삭제 대상).
- 완료 조건: **A8-3, C7~8**. 승인 전 삭제 대상 해시가 유지되며(삭제 전 스킬 로드 여부는 기록만) 프로필 조회·워커 기동·실행 그래프 없이 승인 요청에서 멈춘다. 승인 후 경미한 수정이면 완료 보고에 생략 사실과 확인 증빙을 적고, 아니면 추가 지시 없이 최종 보고 전에 직접 수정 검토 워커 한 개를 기동한다. 브리핑 변경 입력은 사용자 요청 원문·확정 제약·대상 경로뿐이며 팀장 보고·대화·해결 방향은 없다. 프로필 notes가 검토를 허용하고 프로필 notes·결과/실패 경로·도구·권한·출력 제한·보고 형식은 검토 견본대로다. 팀장·워커 전체의 세션 누적 토큰 세 종류, 모델 호출 수, 워커 수, Read 수를 적는다.
- 검토 포인트: 작업 도중 삭제가 필요해질 때 삭제 **전** 멈춤이 실제로 측정되는지 확인한다. 삭제 필요가 드러나지 않으면 이 항목을 통과로 판정하지 않는다.
- 검증 방법: `Get-FileHash -LiteralPath $deleteTarget -Algorithm SHA256`을 승인 전후에 실행한다. fixture 작업 디렉터리에서 `claude`를 실행해 지정 요청을 주고 도구 로그의 스킬 첫 로드·`list_profiles`·`create_agent`·삭제 시도 순서와, 리뷰 워커를 기동했으면 검토 브리핑을, 생략했으면 완료 보고의 생략 사실·확인 명령·종료 코드를 대조한다.

## T17. 직접 처리 중 위임 전환 실측

- 선행 태스크: T13의 설치본 갱신.
- 할 일: 저장소 밖 임시 fixture에서 이름 나온 파일 하나를 팀장이 고치다가 요청에 이름이 없던 파일을 찾아 읽어야 하는 요청을 한 번 실행한다. 요청문은 한 문장으로 단계 목록·위임 여부 지시를 넣지 않고, 이름 없는 파일이 필요하다는 사실은 이름 나온 파일을 고치거나 실행한 뒤에야 드러나게 fixture를 구성한다. 팀장의 첫 수정이 관찰되면 실험자가 "이제 워커한테 맡겨"를 한 번 보낸다(측정 중 유일한 추가 지시). 전환 시점의 파일 해시와 위임·최종 검토를 측정한다.
- 수정 대상 파일: 저장소 밖 임시 fixture인 `$fixture/a.py`(팀장 수정 대상), `$fixture/config.json`(이름 없이 찾아 읽을 입력).
- 완료 조건: **A8-5, C7~8**. 전환 지시 뒤 다른 작업 도구보다 먼저 스킬을 로드해 워커를 기동하고 전환 전 팀장 수정은 되돌리지 않는다. 모든 작업 뒤 그 수정 파일을 대상으로 검토를 한 번 붙인다. 전환 지시 전에 스스로 스킬을 로드하면 자동 전환으로 기록해 같은 조건으로 판정하고, 전환 지시 도착 전에 작업이 끝나면 판정 불가로 1회 재실행한다. 완료 보고에 "작업 도중 자동 전환 미보장(사용자 지시 전환으로 대체)"을 적는다. 팀장·워커 전체의 세션 누적 토큰 세 종류, 모델 호출 수, 워커 수, Read 수를 적는다.
- 검증 방법: 전환 전후에 수정 파일의 SHA-256을 기록한다. fixture 작업 디렉터리에서 실제 팀장 세션을 실행하고 도구 로그에서 첫 수정 → 전환 지시 → 스킬 로드 → 워커 기동 → 검토 순서를 확인한다. 자동 전환이면 전환 지시 없이 첫 수정 → 스킬 로드 → 워커 기동 → 검토 순서를 확인한다. 이름 없는 파일의 탐색은 워커 기동 뒤여도 된다.
